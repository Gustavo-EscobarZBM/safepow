...
| SP1 | Etapa 1.1 **concluída e mesclada em `main` em 2026-09-21**. Sub-etapa 1.2.1 (F18+hardening) **concluída e mesclada em `main` em 2026-09-22**. Sub-etapa 1.2.2 (migrations `ProductPriceHistory`+`LossValuationSnapshot`) **executada em 2026-09-23** no branch `feat/cadastros-sp1-etapa-1-2-2` (4 commits, revisão final feita, 3 achados importantes corrigidos) — aguardando decisão de merge. | [SP1](2026-09-20-sp1-fundacao-de-dados-design.md) | 1.1: [plano](../plans/2026-09-20-sp1-etapa-1-1-harness-integracao.md) (executado). 1.2.1: [plano](../plans/2026-09-21-sp1-etapa-1-2-1-f18-e-hardening.md) (executado). 1.2.2: [plano](../plans/2026-09-22-sp1-etapa-1-2-2-historico-preco-valor-congelado.md) (executado). 1.2.3-1.2.4, 1.3, 1.4: planos ainda não escritos | Baselines atuais: backend 18 suítes/110 testes unitários, 7 arquivos/32 testes de integração (`npm run test:int`), tsc limpo. **Levar para o plano da 1.2.3** (achados menores da revisão da 1.2.2): trigger de fallback deve usar `COALESCE` por coluna (hoje, se só preço ou só custo vier NULL, sobrescreve os dois); importação deve gravar `set_config('app.change_source','import')` (até lá, o histórico registra `manual`); regressão F1 ponta a ponta (spec do SP1 §4.4); qualificar schema/`SET search_path` nas funções de trigger; aplicar 11000/12000 no banco real em janela de manutenção (ALTER TABLE toma ACCESS EXCLUSIVE em `products`/`losses` até o commit). F18 corrigido no código; **ainda não aplicado no banco de desenvolvimento** (autorização pendente). Flutter em `C:\flutter\bin` (fora do PATH). Sistema verificado funcionando de ponta a ponta em 2026-09-22 (véspera de apresentação) — ver seção 10 da memória do projeto. Cada sub-etapa seguinte só ganha plano depois de a anterior fechar |
| SP2 | Não iniciado | — | — | |
| SP3 | Não iniciado | — | — | |
| SP4 | Não iniciado | — | — | |
| SP5 | Não iniciado | — | — | |
