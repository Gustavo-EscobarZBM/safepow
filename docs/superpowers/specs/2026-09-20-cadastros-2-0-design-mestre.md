...
| SP1 | Etapa 1.1 **concluída e mesclada em `main` em 2026-09-21**. Sub-etapa 1.2.1 (F18+hardening) **concluída e mesclada em `main` em 2026-09-22**. Sub-etapa 1.2.2 (migrations `ProductPriceHistory`+`LossValuationSnapshot`) **concluída e mesclada em `main` em 2026-09-23** (5 commits, fast-forward; revisão final feita, 3 achados importantes corrigidos). Sub-etapa 1.2.3 (backend do valor congelado) **concluída e mesclada em `main` em 2026-09-23** (revisão final feita, 1 achado importante corrigido — eco do datetime-local do painel recalculava o valor). Sub-etapa 1.2.4 (linha do tempo de preços no painel) **concluída e mesclada em `main` em 2026-09-23** (revisão final feita, 1 achado importante corrigido; verificada no navegador) — **etapa 1.2 concluída**. Migrations 10000–13000 aplicadas no banco de desenvolvimento em 2026-09-23. Etapa 1.3 dividida em 1.3.1 (backend) e 1.3.2 (web); **1.3.1 concluída e mesclada em `main` em 2026-09-23** (busca com 100 mil produtos abaixo de 300 ms — sem `pg_trgm`; revisão final feita, 1 achado importante corrigido — `page` enorme dava 500). **1.3.2 (web) concluída e mesclada em `main` em 2026-09-24** (verificada no navegador; revisão final feita, 1 achado importante corrigido — oferta de reativar continuava valendo após mudar o código) — **etapa 1.3 concluída**. Etapa 1.4 dividida em 1.4.1 (backend) e 1.4.2 (app); **1.4.1 concluída e mesclada em `main` em 2026-09-24**; migration 14000 aplicada no banco de desenvolvimento com autorização (revisão final sem achados críticos/importantes). **1.4.2 (app) concluída em 2026-09-24** (revisão final feita, 1 achado importante corrigido — gravação em batch; verificação no emulador parcial: migração v1→v2 com perdas intactas comprovada, sync pós-login pendente de login do usuário). **SP1 CONCLUÍDO.** Próximo: SP2 (conforme a ordem do mestre). Menores abertos da 1.4.1: `since`/`after` em formatos exóticos ainda dão 500 (validar com `isNaN(new Date())`); índice 14000 sem `CONCURRENTLY` entra na janela de manutenção do deploy. | [SP1](2026-09-20-sp1-fundacao-de-dados-design.md) | 1.1: [plano](../plans/2026-09-20-sp1-etapa-1-1-harness-integracao.md) (executado). 1.2.1: [plano](../plans/2026-09-21-sp1-etapa-1-2-1-f18-e-hardening.md) (executado). 1.2.2: [plano](../plans/2026-09-22-sp1-etapa-1-2-2-historico-preco-valor-congelado.md) (executado). 1.2.3: [plano](../plans/2026-09-23-sp1-etapa-1-2-3-backend-valor-congelado.md) (executado). 1.2.4: [plano](../plans/2026-09-23-sp1-etapa-1-2-4-linha-do-tempo-de-precos.md) (executado). 1.3.1: [plano](../plans/2026-09-23-sp1-etapa-1-3-1-ciclo-de-vida-e-busca-backend.md) (executado). 1.3.2: [plano](../plans/2026-09-23-sp1-etapa-1-3-2-ciclo-de-vida-e-busca-web.md) (executado). 1.4.1: [plano](../plans/2026-09-24-sp1-etapa-1-4-1-sync-backend.md) (executado). 1.4.2: [plano](../plans/2026-09-24-sp1-etapa-1-4-2-sync-app.md) (executado). 1.2.3-1.2.4, 1.3, 1.4: planos ainda não escritos | Baselines atuais: backend 21 suítes/151 testes unitários, 17 arquivos/94 testes de integração (`npm run test:int`), tsc limpo; web-panel 37 arquivos/162 testes (`npm test`); app 37 testes (`flutter test`, agora estável em paralelo). **Pendências menores ainda abertas:** índice só em `companyId` em `product_price_history`; teste de backfill conferir FORCE também em `products`/`losses`. **Deploy:** aplicar 10000–13000 no banco real em janela de manutenção (ver resultados da 1.2.3 no spec do SP1, seção 4.2). F18 corrigido no código; **ainda não aplicado no banco de desenvolvimento** (autorização pendente). Flutter em `C:\flutter\bin` (fora do PATH). Sistema verificado funcionando de ponta a ponta em 2026-09-22 (véspera de apresentação) — ver seção 10 da memória do projeto. Cada sub-etapa seguinte só ganha plano depois de a anterior fechar |
| SP2 | Spec **aprovado pelo usuário em 2026-09-24** (U1–U5). Etapas 2.1 auditoria → 2.2 justificativa/aprovações → 2.3 correção retroativa → 2.4 motor. Etapa 2.1 dividida em 2.1.1 (backend) e 2.1.2 (web). **2.1.1 (backend da auditoria) concluída e mesclada em `main` em 2026-09-24** (revisão final feita, 3 achados importantes corrigidos — CSV com fórmulas, data que dava 500, ações do Master sem autor; migrations 15000/16000 aplicadas no banco de desenvolvimento com autorização). **2.1.2 (web: página Auditoria + gaveta Histórico) concluída e mesclada em `main` em 2026-09-24** (verificada no navegador; revisão final feita, 1 achado importante corrigido — resposta atrasada/erro deixavam linhas de outra busca na tabela; 7 menores registrados no plano/ledger) — **etapa 2.1 concluída**. Etapa 2.2 dividida em 2.2.1 (backend) e 2.2.2 (web); **2.2.1 (backend de aprovações) concluída e mesclada em `main` em 2026-09-25** (revisão final feita, 1 achado importante corrigido — aprovar e recusar ao mesmo tempo aplicava a mudança com o pedido "recusado"; agora trava a linha; migration 17000 aplicada no banco de desenvolvimento com autorização). **2.2.2 (web das aprovações) concluída e mesclada em `main` em 2026-09-25** (verificada no navegador; revisão final feita, 2 achados importantes corrigidos — data inalterada aparecendo em pedidos de edição de perda e lista da aba errada em troca rápida/erro) — **etapa 2.2 concluída**. **Etapa 2.3 (correção retroativa) concluída e mesclada em `main` em 2026-09-26** (verificada no navegador; revisão final feita, 2 achados importantes corrigidos — prévia atrasada liberava aplicar com números velhos; "Até = hoje" fazia pedidos expirarem). **Etapa 2.4 (motor de cadastros, só painel) concluída e mesclada em `main` em 2026-09-26** (verificada no navegador; revisão final sem achados críticos/importantes). **SP2 CONCLUÍDO.** Próximo: SP3 (conforme a ordem do mestre: SP1→SP2→SP3→SP4→SP6→SP5→SP7→SP8→SP9). Menores abertos da 2.4: diálogo de edição do cadastro fecha só depois de recarregar a lista; título da gaveta de Histórico some durante a animação; caracterização não fixa ordem dos botões; envio em andamento vaza para a próxima edição (herdado); title redundante no botão Histórico; resource-table sem 'use client'. Limitação conhecida: perda registrada/editada depois dentro de uma janela já corrigida volta a pegar o preço do histórico (o histórico de preço não é editável — decisão do SP1). Menores abertos da 2.3: testes de borda (1 dia depois de "to", modo justificativa); preço com mais de 2 casas/valor enorme; resetar `app.audit_mode` após o UPDATE; diálogo pré-preenche com o preço salvo; aviso atrás do diálogo de edição; aprovador sem o impacto financeiro na fila; snapshot só contagem+somas; arredondamento de exibição. Menores abertos da 2.2.2: decisão que falha com 409/404 não recarrega a lista; sem mensagem de sucesso ao aprovar/recusar; limite vazio dá erro em inglês; selo conta os próprios pedidos; criar pedido não atualiza o selo na hora; aviso "Enviado para aprovação" não some; justificativa se perde ao tentar de novo após erro; código de barras "00123" vs "123" comparado como número; retro_fix ativável sem efeito até a 2.3; master em /aprovacoes vê 403. Menores abertos da 2.2.1: duplo clique no pedido dá 500 (devolver 409); rótulo "Perda de <nome>" > 200 caracteres dá 500; snapshot da perda sem a conferência; pedido vencido substituído vira "cancelled"; faltam testes de aprovar edição de perda/arquivamento até o fim; substituição de pedido sem evento de auditoria. Baselines após a 2.2.1: backend 23 suítes/161 unitários, 27 arquivos/175 de integração. Menores da 2.1.2 abertos: gaveta de Usuários sem os logins e página sem filtro "Pessoa"; gaveta sem aviso de "só as 100 mais recentes"; rótulos faltando (status trial, origem/valuationSource da perda, quantidade pt-BR, "Senha definida" na criação); faixa de 32px no celular; foco ao fechar gavetas/diálogos. Baselines: backend 22 suítes/153 unitários, 22 arquivos/136 de integração; web-panel 42 arquivos/189 testes, tsc limpo. Achado fora do escopo: `auth_lookup_user_by_email` não enxerga usuários com empresa quando o dono das tabelas não é superusuário (ver spec do SP2, fim da seção 2) | [SP2](2026-09-24-sp2-auditoria-aprovacoes-motor-design.md) | 2.1.1: [plano](../plans/2026-09-24-sp2-etapa-2-1-1-auditoria-backend.md) (executado). 2.1.2: [plano](../plans/2026-09-24-sp2-etapa-2-1-2-auditoria-web.md) (executado). 2.2.1: [plano](../plans/2026-09-25-sp2-etapa-2-2-1-aprovacoes-backend.md) (executado). 2.2.2: [plano](../plans/2026-09-25-sp2-etapa-2-2-2-aprovacoes-web.md) (executado). 2.3: [plano](../plans/2026-09-25-sp2-etapa-2-3-correcao-retroativa.md) (executado). 2.4: [plano](../plans/2026-09-26-sp2-etapa-2-4-motor-de-cadastros.md) (executado) | Mudança em relação ao mestre: auditoria por **trigger + eventos explícitos** (U4, substitui A7); `BaseCatalogService` adiado para o SP4 (regra dos três) |
| SP3 | Não iniciado | — | — | |
| SP4 | Não iniciado | — | — | |
| SP5 | Não iniciado | — | — | |
| SP6 | Não iniciado | — | — | |
